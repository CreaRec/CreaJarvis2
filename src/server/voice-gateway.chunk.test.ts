...
describe("chunkBase64Audio", () => {
  ...
  it("splits on multiple of 4", () => {
    ...
    expect(parts.every((p) => p.length % 4 === 0 || p === parts.at(-1))).toBe(
      true,
    );
...
describe("shouldAnnounceRealtimeReady", () => {
  it("announces on first connect", () => {
    expect(shouldAnnounceRealtimeReady(false)).toBe(true);
  });

  it("skips once already announced", () => {
    expect(shouldAnnounceRealtimeReady(true)).toBe(false);
  });
});
