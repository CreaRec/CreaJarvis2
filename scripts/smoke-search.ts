...
import { GooglePlacesClient } from "../src/search/google-places-client.js";
...
async function main(): Promise<void> {
  ...
  if (mode === "places") {
    ...
    console.log(
      `[smoke-search] places query=${JSON.stringify(query)} near=${JSON.stringify(near ?? null)}`,
    );
    const places = new GooglePlacesClient(
      config.GOOGLE_PLACES_API_KEY,
      config.BRAVE_SEARCH_LANG,
    );
    const result = await places.searchPlaces({ query, near, count: 5 });
