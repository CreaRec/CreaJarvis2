 ...
 * Weather: larger bead on ring A (orbits like the red beads, inset from rim).
 */
(function () {
  "use strict";
  ...
  function drawWeatherBadge(ctx, size, weather) {
    ...
    if (label.indexOf("rain") >= 0 || label.indexOf("drizzle") >= 0 || label.indexOf("shower") >= 0) {
      ctx.beginPath();
      ctx.arc(gx - size * 0.05, gy, size * 0.07, 0, Math.PI * 2);
      ctx.arc(gx + size * 0.05, gy - size * 0.01, size * 0.09, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(gx - size * 0.03, gy + size * 0.08);
      ctx.lineTo(gx - size * 0.06, gy + size * 0.16);
      ctx.moveTo(gx + size * 0.04, gy + size * 0.08);
      ctx.lineTo(gx + size * 0.01, gy + size * 0.16);
      ctx.stroke();
    } else if (label.indexOf("snow") >= 0) {
      for (let a = 0; a < 3; a++) {
        const ang = (a * Math.PI) / 3;
        ctx.beginPath();
        ctx.moveTo(gx - Math.cos(ang) * size * 0.09, gy - Math.sin(ang) * size * 0.09);
        ctx.lineTo(gx + Math.cos(ang) * size * 0.09, gy + Math.sin(ang) * size * 0.09);
        ctx.stroke();
      }
    } else if (label.indexOf("clear") >= 0 || label.indexOf("sun") >= 0) {
      ctx.beginPath();
      ...
      for (let i = 0; i < 8; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(gx + Math.cos(ang) * size * 0.09, gy + Math.sin(ang) * size * 0.09);
        ctx.lineTo(gx + Math.cos(ang) * size * 0.14, gy + Math.sin(ang) * size * 0.14);
        ctx.stroke();
      }
    } else {
      ctx.beginPath();
      ctx.arc(gx - size * 0.06, gy + size * 0.02, size * 0.075, 0, Math.PI * 2);
      ctx.arc(gx + size * 0.04, gy, size * 0.095, 0, Math.PI * 2);
      ctx.arc(gx + size * 0.12, gy + size * 0.025, size * 0.07, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    ctx.fillText(String(temp), cx, cy + size * 0.2);
  }

  function makeWeatherBead(orbitRadius) {
    // ~2× the usual 0.12 red beads; inset from the ring rim.
    const badgeSize = 256;
    const badgeCanvas = document.createElement("canvas");
    badgeCanvas.width = badgeSize;
    badgeCanvas.height = badgeSize;
    const badgeCtx = badgeCanvas.getContext("2d");
    drawWeatherBadge(badgeCtx, badgeSize, {
      tempLabel: "+12°",
      label: "partly cloudy",
    });
    ...
    // Opaque billboard — no translucent mesh (that looked empty with particles inside).
    ...
    // Red bead diameter ≈ 0.24; weather bead ≈ 0.5 world units across.
    sprite.scale.set(0.5, 0.5, 1);
    node.add(sprite);
    node.position.set(orbitRadius, 0, 0);
    node.userData = {
  ...
  // Keep both original rings. Weather replaces the red bead on ring A.
  const ringA = makeOrbitalRing(3.5, 0xffffff, 0.35, 0.3);
  const ringB = makeOrbitalRing(4.0, 0xaaaaaa, -0.22, -0.5);
  // Clear ring-A red beads, attach a larger weather bead inset from the rim.
  while (ringA.userData.spinner.children.length) {
    ringA.userData.spinner.remove(ringA.userData.spinner.children[0]);
  }
  const weatherBead = makeWeatherBead(3.5 * 0.86);
  ringA.userData.spinner.add(weatherBead);
  ringA.userData.weather = weatherBead.userData;
  root.add(ringA);
  root.add(ringB);
  ...
  window.setWeather = function setWeather(payload) {
    ...
    const ud = ringA.userData.weather;
    if (!ud) {
      return;
    }
    const weather = {
